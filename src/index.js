...
const typeMessage = async (tpmessage) => {
    /* ks.startBatch()
      .batchTypeText(tpmessage.data[0].value)
      .sendBatch(); */
    //ks.sendText(tpmessage.data[0].value);
      /* robot.typeString(tpmessage.data[0].value).go(); */
    //await keyboard.type(tpmessage.data[0].value);
    //robot.typeStringDelayed(tpmessage.data[0].value,1000);
    robot.typeString(tpmessage.data[0].value).go();
}
...
TPClient.on("Close", (data) => {
    //TP Is connected now
    logIt('DEBUG','We are closing, turn off Jar');
    robot.stopJar();
});
